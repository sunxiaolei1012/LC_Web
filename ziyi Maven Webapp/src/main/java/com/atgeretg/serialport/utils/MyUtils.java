package com.atgeretg.serialport.utils;
...
//	}
	...
	public static String byteArray2HexString_int(byte[] arrBytes, int count, boolean blank) {
		String ret = "";
		if (arrBytes == null || arrBytes.length < 1)
			return ret;
		if (count > arrBytes.length)
		...
		for (int i = 0; i < count; i++) {
			ret = Integer.toHexString(arrBytes[i] & 0xFF).toUpperCase();
			if (ret.length() == 1)
				builder.append("0").append(Integer.parseInt(ret,16)+"");
			else
				builder.append(Integer.parseInt(ret,16)+"");
			if (blank)
				builder.append(" ");
		}
	...
	public static String byteArray2HexString_char(byte[] arrBytes, int count, boolean blank) {
		String ret = "";
		if (arrBytes == null || arrBytes.length < 1)
			return ret;
		if (count > arrBytes.length)
		...
		for (int i = 0; i < count; i++) {
			ret = Integer.toHexString(arrBytes[i] & 0xFF).toUpperCase();
			if (ret.length() == 1)
				builder.append("0").append((char)Integer.parseInt(ret,16));
			else
				builder.append((char)Integer.parseInt(ret,16));
			if (blank)
				builder.append(" ");
		}
